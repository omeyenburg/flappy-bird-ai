...
class Agent:
    ...
    def load():
        ...
        return Agent(layers, weights, biases, activation)
    ...
    def train(self):
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            while True:
                ...
                self.weights = [self.weights[agent][:] for agent in new_agents]
                self.biases = [self.biases[agent][:] for agent in new_agents]
